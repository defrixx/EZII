import hashlib, ipaddress, json, logging, socket, time, uuid
...
from app.services.ingestion_service import finish_upload_job
...
def provider_for(model:ModelEndpoint,db:Session):
    conn=db.get(ModelConnection,model.connection_id)
...
def create_kb(p:KBIn,db:Session=Depends(db_dep)):
    ...
    validate_kb_settings(p.model_dump())
    if p.is_default:
        for x in db.scalars(select(KnowledgeBase).where(KnowledgeBase.is_default.is_(True))): x.is_default=False
    row=KnowledgeBase(**p.model_dump()); db.add(row); commit_or_conflict(db,"knowledge_base_name_exists"); db.refresh(row); return row
...
def update_kb(id:uuid.UUID,p:KBPatch,db:Session=Depends(db_dep)):
    ...
    if old!=row.embedding_model_id: row.reindex_required=True
...
async def add_site(kb_id:uuid.UUID,p:SiteIn,db:Session=Depends(db_dep)):
    ...
    resolved=set()
    try:
        for x in socket.getaddrinfo(parsed.hostname,parsed.port or (443 if parsed.scheme=="https" else 80)):
            ip=ipaddress.ip_address(x[4][0]);resolved.add(str(ip))
    ...
    async with httpx.AsyncClient(timeout=20,follow_redirects=False,trust_env=False) as c:
        async with c.stream("GET",p.url,headers={"User-Agent":"EZII-knowledge-ingestion","Accept":"text/html,text/plain"}) as response:
    ...
    decoded=bytes(data).decode(response.encoding or "utf-8",errors="replace");text=BeautifulSoup(decoded,"html.parser").get_text(" ",strip=True)
    ...
    for i,start in enumerate(range(0,len(text),650)): db.add(DocumentChunk(document_id=row.id,chunk_index=i,content=text[start:start+900],token_count=max(1,len(text[start:start+900])//4)))
...
async def answer(chat_id:uuid.UUID,p:MessageIn,db:Session=Depends(db_dep)):
    ...
    chunks=list(db.scalars(select(DocumentChunk).join(Document).where(Document.knowledge_base_id==kb.id,Document.source_type.in_(kb.knowledge_mode),Document.status=="approved",Document.enabled_in_retrieval.is_(True)).limit(100)))
    ...
    ranked=[];warning_codes=[];vector_ranked=False
    if kb.embedding_model_id and not kb.reindex_required:
        em=db.get(ModelEndpoint,kb.embedding_model_id)
        if em:
            try:
                query_vector=(await provider_for(em,db).embeddings(em.model_id,[p.content]))[0];hits=VectorService().search(em.id,kb.id,query_vector);hit_ids=[x["payload"].get("chunk_id","") for x in hits];hit_rows=list(db.scalars(select(DocumentChunk).join(Document).where(DocumentChunk.id.in_(hit_ids),Document.knowledge_base_id==kb.id,Document.source_type.in_(kb.knowledge_mode),Document.status=="approved",Document.enabled_in_retrieval.is_(True))));by_id={str(x.id):x for x in hit_rows};ranked=[by_id[x] for x in hit_ids if x in by_id];vector_ranked=bool(ranked)
            except Exception as exc:
    ...
    if not vector_ranked:
        terms=[word.lower() for word in p.content.split() if len(word)>2];scored=[(sum(chunk.content.lower().count(word) for word in terms),chunk) for chunk in chunks];ranked=[chunk for score,chunk in sorted(scored,key=lambda item:item[0],reverse=True)[:5] if score>0]
    matching_terms=[x for x in glossary_rows if x.term.lower() in p.content.lower() or any(s.lower() in p.content.lower() for s in x.synonyms)];context="\n\n".join([*(f"TERM: {x.term}\nDEFINITION: {x.definition}" for x in matching_terms),*(x.content for x in ranked)])[:kb.context_token_budget*4]
    if not kb.chat_model_id: result="База знаний не настроена: выберите chat-модель в панели управления." if p.locale=="ru" else "The knowledge base is not configured: choose a chat model in Manage."
        ...
        result=await provider.answer(model.model_id,[{"role":"system","content":f"Answer only from the supplied knowledge context. Cite uncertainty clearly. Response tone: {kb.response_tone}.\nCONTEXT:\n"+context},*selected_history,{"role":"user","content":p.content}]);token_usage=provider.last_usage
    source_ids=list(dict.fromkeys(str(x.document_id) for x in ranked));source_documents=[db.get(Document,source_id) for source_id in source_ids];source_documents=[source for source in source_documents if source and source.knowledge_base_id==kb.id];source_types=list(dict.fromkeys(source.source_type for source in source_documents));source_refs=[{"id":str(source.id),"title":source.title,"source_type":source.source_type} for source in source_documents]
    out=Message(chat_id=chat.id,role="assistant",content=result,source_types=source_types,metadata_json={"source_ids":source_ids,"sources":source_refs,"warning_codes":warning_codes});db.add(out);db.add(ResponseTrace(knowledge_base_id=kb.id,chat_id=chat.id,model_endpoint_id=kb.chat_model_id,answer_mode="grounded" if context else "model_only",source_types=source_types,source_ids=source_ids,ranking_scores={},token_usage=token_usage,warning_codes=warning_codes,latency_ms=(time.perf_counter()-started)*1000,status="degraded" if warning_codes else "ok"));chat.updated_at=datetime.now(timezone.utc);db.commit();db.refresh(out);return out
