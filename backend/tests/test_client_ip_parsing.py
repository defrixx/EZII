...
def test_rate_limit_uses_right_most_untrusted_forwarded_for_ip():
    request = _request([(b"x-forwarded-for", b"1.1.1.1, 2.2.2.2")])
    assert _client_ip(request) == "2.2.2.2"


def test_auth_request_ip_uses_right_most_untrusted_forwarded_for_ip():
    request = _request([(b"x-forwarded-for", b"1.1.1.1, 2.2.2.2")])
    assert _request_ip(request) == "2.2.2.2"
